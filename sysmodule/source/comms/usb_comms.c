...
static Result _usbCommsModInterfaceInit1x(u32 intf_ind, const UsbCommsModInterfaceInfo *info);
static Result _usbCommsModInterfaceInit5x(u32 intf_ind, const UsbCommsModInterfaceInfo *info);
static Result _usbCommsModInterfaceInit(u32 intf_ind, const UsbCommsModInterfaceInfo *info);

static Result _usbCommsModWrite(usbCommsInterface *interface, const void* buffer, size_t size, size_t *transferredSize, AbortCallbackFn callback);

static void _usbCommsModUpdateInterfaceDescriptor(struct usb_interface_descriptor *desc, const UsbCommsModInterfaceInfo *info) {
    if (info != NULL) {
    ...
    if (g_usbCommsInitialized) {
        ...
        if (R_SUCCEEDED(rc)) {
            if (hosversionAtLeast(5,0,0)) {
                ...
                // Send product
                if (R_SUCCEEDED(rc)) rc = usbDsAddUsbStringDescriptor(&iProduct, "EdiZon");
                // Send serial number
            ...
            if (R_SUCCEEDED(rc)) {
                for (u32 i = 0; i < num_interfaces; i++) {
                    ...
                    rc = _usbCommsModInterfaceInit(i, infos == NULL ? NULL : infos + i);
...
{
    return usbCommsModInitializeEx(1, NULL);
}

static void _usbCommsModInterfaceFree(usbCommsInterface *interface)
{
...
void usbCommsModExit(void)
{
    ...
    {
        _usbCommsModInterfaceFree(&g_usbCommsInterfaces[i]);
    }
}
...
{
    if (hosversionAtLeast(5,0,0)) {
        return _usbCommsModInterfaceInit5x(intf_ind, info);
    } else {
        return _usbCommsModInterfaceInit1x(intf_ind, info);
    }
}
...
static Result _usbCommsModInterfaceInit5x(u32 intf_ind, const UsbCommsModInterfaceInfo *info)
{
    ...
    _usbCommsModUpdateInterfaceDescriptor(&interface_descriptor, info);
...
static Result _usbCommsModRead(usbCommsInterface *interface, void* buffer, size_t size, size_t *transferredSize, AbortCallbackFn callback)
{
    Result rc=0;
    u32 urbId=0;
    u8 *bufptr = (u8*)buffer;
    ...
    while(size)
    {
        ...
        while (true) {
            if (R_SUCCEEDED(eventWait(&interface->endpoint_in->CompletionEvent, 1)))
                break;
            
            if (callback())
                return KernelError_TimedOut;
...
size_t usbCommsModReadEx(void* buffer, size_t size, u32 interface, AbortCallbackFn callback)
{
    ...
    rc = _usbCommsModRead(inter, buffer, size, &transferredSize, callback);
    ...
    if (R_FAILED(rc)) {
        ...
        if (R_SUCCEEDED(rc2)) {
            if (state!=5) {
                rwlockWriteLock(&inter->lock_out);
                rc = _usbCommsModRead(&g_usbCommsInterfaces[interface], buffer, size, &transferredSize, callback); //If state changed during transfer, try again. usbDsWaitReady() will be called from this.
                rwlockWriteUnlock(&inter->lock_out);
            }
        }
...
{
    return usbCommsModReadEx(buffer, size, 0, callback);
}
...
size_t usbCommsModWriteEx(const void* buffer, size_t size, u32 interface, AbortCallbackFn callback)
{
    ...
    rc = _usbCommsModWrite(&g_usbCommsInterfaces[interface], buffer, size, &transferredSize, callback);
    ...
    if (R_FAILED(rc)) {
        ...
        if (R_SUCCEEDED(rc2)) {
            if (state!=5) {
                rwlockWriteLock(&inter->lock_in);
                rc = _usbCommsModWrite(&g_usbCommsInterfaces[interface], buffer, size, &transferredSize, callback); //If state changed during transfer, try again. usbDsWaitReady() will be called from this.
                rwlockWriteUnlock(&inter->lock_in);
            }
        }
...
{
    return usbCommsModWriteEx(buffer, size, 0, callback);
}